  ...
  implements Listener
{
	...
	public void onPlayerQuit(ReturnToLobbyEvent e) {
		 ...
		 player.sendMessage("�7Gain total de �bCr�dits �7sur la partie : �e" + String.format("%.2f", pl.getGainedLC()));
		 player.sendMessage("�6-----------------------------------------------------");
	}
      ...
      if (slot < 2) {
        ...
        if (target.getType() == Material.DIAMOND_ORE) {
          ...
          if (gun.getLevel() <= gun.getGun().getMaxLevel() - 1) {
            int level = gun.getLevel() + 1;
            GunWeaponTmpl.GunLevel gunLevel = gun.getGun().getGunLevel(level);
            Utils.sendActionBar(player, "�2Am�liorez votre " + gun.getGun().getName() + " �2au niveau �a" + level + " �2pour �e" + gunLevel.getPrice() + "�3$");
          }
        }
        else if (target.getType() == Material.ENDER_CHEST) {
          Utils.sendActionBar(player, "�2Achetez une arme al�atoire pour �e250�3$");
        }
      }
        ...
        if ((target.getX() == fBlock.getX()) && (target.getY() == fBlock.getY()) && (target.getZ() == fBlock.getZ())) {
          ...
          if (weapon != null) {
            ...
            if ((weapon instanceof GunWeaponTmpl)) {
              ...
              show.append("�r : �4D�g�ts: �c").append(level.getMinDamage()).append("�1/�c").append(level.getMaxDamage());
            }
